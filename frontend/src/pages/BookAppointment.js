...
function BookAppointment() {
  ...
  const handleDateSelect = async (dateStr, dateObj) => {
    setSelectedDate(dateStr); setSelectedDateObj(dateObj);
    setSelectedSlot(''); setSlots([]); setSlotsLoading(true); setMessage(null);
    try {
      const res = await getSlots(dateStr);
      setSlots(res.data.slots); setStep(2);
    } catch (err) {
      setMessage({ type:'error', text: err.response?.data?.message || 'Could not load slots. Please try again.' });
    } finally { setSlotsLoading(false); }
  };

  const handleSlotSelect = (slot) => { setSelectedSlot(slot); setStep(3); };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setMessage({ type:'success', text: res.data.message }); setStep(4);
    } catch (err) {
  ...
  const resetForm = () => {
    setStep(1); setSelectedDate(''); setSelectedDateObj(null);
    setSelectedSlot(''); setSlots([]); setForm({ name:'', phone:'', purpose:'' }); setMessage(null);
  };
    ...
    <div className="appt-page">
      ...
      <section className="section">
        <div className="appt-container">
            ...
            <div className="step-content">
              ...
              <div className="date-grid">
                {wednesdays.map((wed,i) => {
                  ...
                  return (
                    <button key={i} className={`date-btn${selectedDate === dateStr ? ' selected' : ''}`}
              ...
              {slotsLoading && <p className="loading-text"><span className="spinner" style={{ borderTopColor:'var(--primary)', borderColor:'var(--border)', width:18, height:18 }}></span>&nbsp;Loading slots...</p>}
            </div>
          )}

          {step === 2 && (
            <div className="step-content">
              <button className="back-btn" onClick={() => setStep(1)}><i className="fas fa-arrow-left"></i> Back</button>
              <h3 className="step-heading">Available slots for {selectedDateObj ? formatDisplay(selectedDateObj) : selectedDate}</h3>
              <div className="slots-grid">
                {slots.map(({ slot, available }, i) => (
                  <button key={i}
                    className={`slot-btn ${available ? 'available' : 'taken'}${selectedSlot === slot ? ' selected' : ''}`}
                    onClick={() => available && handleSlotSelect(slot)} disabled={!available}
              ...
              {slots.length > 0 && slots.every(s => !s.available) && (
                <div className="alert alert-error" style={{ marginTop:20 }}>
                  Sorry, all slots for this date are fully booked. Please choose another date.
                </div>
              )}
            </div>
          )}
          ...
          {step === 3 && (
            <div className="step-content">
              <button className="back-btn" onClick={() => setStep(2)}><i className="fas fa-arrow-left"></i> Back</button>
              ...
              <form onSubmit={handleSubmit} style={{ marginTop:20 }}>
                ...
                <div className="form-group">
                  ...
                  <input type="tel" placeholder="+254 7XX XXX XXX" required value={form.phone} onChange={e => setForm({ ...form, phone: e.target.value })} />
                </div>
                <div className="form-group">
                  <label>Purpose <span style={{ fontWeight:400, color:'var(--text-light)' }}>(optional)</span></label>
                  <textarea rows="3" placeholder="Briefly describe your reason..." value={form.purpose} onChange={e => setForm({ ...form, purpose: e.target.value })}></textarea>
                </div>
                <button type="submit" className="btn btn-primary" style={{ width:'100%', padding:'14px' }} disabled={loading}>
                  {loading ? <><span className="spinner"></span> Booking...</> : <><i className="fas fa-check"></i> Confirm Appointment</>}
                </button>
              </form>
            </div>
          )}
